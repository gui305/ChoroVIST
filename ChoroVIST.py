...
class ChoroidApp(tk.Tk):
    def __init__(self):
        ...
        self.iconbitmap(os.path.join(base_path, "images/coroide.ico"))
    ...
    def show_welcome_moderno(self):
        ...
        # Carregar logotipos
        try:
            logo_faculdade = Image.open(os.path.join(base_path, 'images/Tecnico.png')).resize((280, 180))
            self.logo_faculdade_img = ImageTk.PhotoImage(logo_faculdade)
        except:
            self.logo_faculdade_img = None

        try:
            logo_hospital = Image.open(os.path.join(base_path, 'images/FMUL.png')).resize((200, 200))
            self.logo_hospital_img = ImageTk.PhotoImage(logo_hospital)
        except:
            self.logo_hospital_img = None

        try:
            logo_app = Image.open(os.path.join(base_path, 'images/OLHO.png')).resize((230, 230))
